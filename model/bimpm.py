...
class BiMPM(object):
    ...
    def build_placeholder(self):
        with tf.variable_scope(name_or_scope=self.scope):
            ...
            self.num_classes = self.args.class_num
            self.batch_size = self.args.batch_size

            # ---- place holder -----
            self.sent1_token = tf.placeholder(tf.int32, [None, self.seq_len], name='sent1_token')
            self.sent2_token = tf.placeholder(tf.int32, [None, self.seq_len], name='sent2_token')
