...
class ARC_I(ModelTemplate):
    def __init__(self,args,scope):
        super(ARC_I, self).__init__(args,scope)
        self.args=args
        self.scope=scope
    ...
    def build_model(self):
        with tf.device('/device:GPU:%s' % self.gpu_id):
            ...
            if self.args.match_fun=='cosine':
                s1_emb=tf.layers.flatten(s1_emb)
                s2_emb=tf.layers.flatten(s2_emb)
