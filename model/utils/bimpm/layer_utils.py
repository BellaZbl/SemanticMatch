...
def my_lstm_layer(input_reps, lstm_dim, input_lengths=None, scope_name=None, reuse=False, is_training=True,
    ...
    #                         lambda:dropout_rate,
    #                         lambda:0.0)
    input_reps=tf.nn.dropout(input_reps,1.0-dropout_rate)
...
def dropout_layer(input_reps, dropout_rate, is_training=True):
    ...
    output_repr = tf.cond(is_training, 
                        lambda:tf.nn.dropout(input_reps, (1 - dropout_rate)),
                        lambda:input_reps)
...
def calcuate_attention(in_value_1, in_value_2, feature_dim1, feature_dim2, scope_name='att',
    ...
    in_value_1 = tf.nn.dropout(in_value_1, 1.0-dropout_rate)
    in_value_2 = tf.nn.dropout(in_value_2, 1.0-dropout_rate)
    ...
    with tf.variable_scope(scope_name):
        ...
        s=tf.reshape(tensor=in_value_1,shape=(batch_size*len_1,feature_dim1))
        atten_value_1 = tf.matmul(s, atten_w1)  # [batch_size*len_1, feature_dim]
        atten_value_1 = tf.reshape(atten_value_1, [batch_size, len_1, att_dim])
        atten_value_2 = tf.matmul(tf.reshape(in_value_2, [-1, feature_dim2]), atten_w2)  # [batch_size*len_2, feature_dim]
