...
def slstm_cell( scope, hidden_size,
    ...
    for i in range(num_layers):
        ...
        #get states before
        initial_hidden_states_before=[tf.reshape(get_hidden_states_before(initial_hidden_states, step+1, shape, hidden_size), [-1, hidden_size]) for step in range(step)]
        initial_hidden_states_before=sum_together(initial_hidden_states_before)
        initial_hidden_states_after= [tf.reshape(get_hidden_states_after(initial_hidden_states, step+1, shape, hidden_size), [-1, hidden_size]) for step in range(step)]
        initial_hidden_states_after=sum_together(initial_hidden_states_after)
        #get states after
        initial_cell_states_before=[tf.reshape(get_hidden_states_before(initial_cell_states, step+1, shape, hidden_size), [-1, hidden_size]) for step in range(step)]
        initial_cell_states_before=sum_together(initial_cell_states_before)
        initial_cell_states_after=[tf.reshape(get_hidden_states_after(initial_cell_states, step+1, shape, hidden_size), [-1, hidden_size]) for step in range(step)]
