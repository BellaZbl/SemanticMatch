...
def CNN_layer(variable_scope, x1, x2, s, d, w, di, model_type, l2_reg, reuse=False):
    # x1, x2 = [batch, d, s, 1]
    with tf.variable_scope(variable_scope, reuse=reuse):
        if model_type == "ABCNN1" or model_type == "ABCNN3":
            with tf.name_scope("att_mat"):
                aW_left = tf.get_variable(name="aW_left", shape=(s, d),
                                          initializer=tf.contrib.layers.xavier_initializer(),
                                          regularizer=tf.contrib.layers.l2_regularizer(scale=l2_reg))
                aW_right = tf.get_variable(name="aW_right", shape=(s, d),
                                           initializer=tf.contrib.layers.xavier_initializer(),
                                           regularizer=tf.contrib.layers.l2_regularizer(scale=l2_reg))
                # [batch, s, s]
                ...
                x1_a = tf.expand_dims(tf.matrix_transpose(tf.einsum("ijk,kl->ijl", att_mat, aW_left)), -1)
                x2_a = tf.expand_dims(tf.matrix_transpose(tf.einsum("ijk,kl->ijl", tf.matrix_transpose(att_mat), aW_right)),
                                      -1)
        ...
        left_conv = convolution(name_scope="left", x=pad_for_wide_conv(x1, w), d=d, di=di, w=w, l2_reg=l2_reg,
                                reuse=False)
        right_conv = convolution(name_scope="right", x=pad_for_wide_conv(x2, w), d=d, di=di, w=w, l2_reg=l2_reg,
                                 reuse=True)
        ...
        right_wp = w_pool(variable_scope="right", x=right_conv, attention=right_attention, s=s, w=w,
